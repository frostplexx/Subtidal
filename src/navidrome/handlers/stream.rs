...
//     request distinguishes the two cases.
...
type ManifestCache = HashMap<(u64, Quality), (Instant, StreamInfo)>;
static MANIFEST_CACHE: LazyLock<Mutex<ManifestCache>> =
    LazyLock::new(|| Mutex::new(ManifestCache::new()));

fn cached_manifest(track_id: u64, tier: Quality) -> Option<StreamInfo> {
    let mut map = MANIFEST_CACHE.lock().unwrap();
    map.retain(|_, (at, _)| at.elapsed() < MANIFEST_TTL);
    map.get(&(track_id, tier)).map(|(_, i)| i.clone())
}

fn store_manifest(track_id: u64, tier: Quality, info: &StreamInfo) {
    let mut map = MANIFEST_CACHE.lock().unwrap();
    map.retain(|_, (at, _)| at.elapsed() < MANIFEST_TTL);
    map.insert((track_id, tier), (Instant::now(), info.clone()));
}
...
// so a long queue cannot grow this without limit. Deliberately in
// memory and lost on restart — it is a latency cache, not storage.
const AUDIO_TTL: Duration = Duration::from_secs(300);
const MAX_CACHE_BYTES: usize = 256 * 1024 * 1024;
type AudioCache = HashMap<(u64, Quality), (Instant, Arc<Vec<u8>>)>;
static AUDIO_CACHE: LazyLock<Mutex<AudioCache>> = LazyLock::new(|| Mutex::new(AudioCache::new()));
...
type SizeCache = HashMap<(u64, Quality), Arc<Vec<u64>>>;
static SIZE_CACHE: LazyLock<Mutex<SizeCache>> = LazyLock::new(|| Mutex::new(SizeCache::new()));
...
const MAX_SEGMENT_CACHE_BYTES: usize = 256 * 1024 * 1024;
type SegmentCache = HashMap<String, (Instant, bytes::Bytes)>;
static SEGMENT_CACHE: LazyLock<Mutex<SegmentCache>> =
    LazyLock::new(|| Mutex::new(SegmentCache::new()));

// Fetch one part, serving it from the segment cache when warm.
async fn fetch_part(url: &str) -> Result<bytes::Bytes, Error> {
    {
        let mut map = SEGMENT_CACHE.lock().unwrap();
        map.retain(|_, (at, _)| at.elapsed() < SEGMENT_TTL);
        if let Some((_, bytes)) = map.get(url) {
            return Ok(bytes.clone());
        }
    }
    let bytes = crate::tidal::client().fetch_one(url).await?;
    let mut map = SEGMENT_CACHE.lock().unwrap();
    map.insert(url.to_string(), (Instant::now(), bytes.clone()));
    // Evict oldest-first until back under the cap.
    let mut total: usize = map.values().map(|(_, b)| b.len()).sum();
    while total > MAX_SEGMENT_CACHE_BYTES && map.len() > 1 {
        let Some(oldest) = map
            .iter()
        ...
        if let Some((_, b)) = map.remove(&oldest) {
            total -= b.len();
        }
    }
    Ok(bytes)
...
// One part of a progressive response: the URL to fetch and the slice of
// it that falls inside the requested byte range. Interior parts are
// taken whole; only the first and last are trimmed.
struct Part {
    url: String,
    skip: usize,
    take: usize,
...
// Which parts cover [start, end], and how much of each. `sizes` and
// `urls` are parallel, both starting with the init header — the
// concatenated file is exactly init followed by the segments, so a byte
// offset needs no special handling for it.
fn plan(urls: &[String], sizes: &[u64], start: u64, end: u64) -> Vec<Part> {
...
fn cached_audio(track_id: u64, tier: Quality) -> Option<Arc<Vec<u8>>> {
    let mut map = AUDIO_CACHE.lock().unwrap();
    map.retain(|_, (at, _)| at.elapsed() < AUDIO_TTL);
    map.get(&(track_id, tier)).map(|(_, b)| Arc::clone(b))
}

fn store_audio(track_id: u64, tier: Quality, bytes: &Arc<Vec<u8>>) {
    let mut map = AUDIO_CACHE.lock().unwrap();
    map.retain(|_, (at, _)| at.elapsed() < AUDIO_TTL);
    map.insert((track_id, tier), (Instant::now(), Arc::clone(bytes)));
    // Evict oldest-first until back under the cap. A single track larger
    // than the cap would otherwise sit there alone, so the last entry is
    // never evicted — it is the one just asked for.
    let mut total: usize = map.values().map(|(_, b)| b.len()).sum();
    while total > MAX_CACHE_BYTES && map.len() > 1 {
        let Some(oldest) = map
            .iter()
            .min_by_key(|(_, (at, _))| *at)
            .map(|(k, _)| *k)
...
async fn serve(
    ...
    match info.asset {
        ...
        Asset::Segmented { init, segments } => {
            ...
            // network at all and makes ranges trivial.
            if let Some(bytes) = cached_audio(track_id, requested) {
                return audio_reply(bytes, range, attachment);
            }
            // FLAC is rewrapped out of its MP4 fragments rather than
            // concatenated as fragmented MP4. A concatenated fMP4 has no
            // duration and no segment index, so a player must download
            // and scan the whole file before it can decode a frame —
            // which is exactly the multi-second "Loading…" this avoids.
            // A native FLAC stream is decodable from byte 0.
            if info.codec.starts_with("flac") {
                return flac_reply(track_id, requested, init, segments, range, attachment).await;
            }
            // Otherwise start sending as soon as the first segment
            // arrives, rather than making the client wait for the whole
            // track. Needs the size table for an accurate
            // Content-Length; without one, seeking would break, so fall
            // back to buffering instead.
            let urls: Vec<String> = std::iter::once(init.clone())
                .chain(segments.iter().cloned())
                .collect();
            if let Some(sizes) = sizes_for(track_id, requested, &urls).await {
                return progressive_reply(track_id, requested, urls, sizes, range, attachment);
            }
            tracing::debug!(
                "stream {track_id}: no segment size table; buffering the whole track"
            );
            let count = segments.len();
            match assemble(track_id, requested, init, segments).await {
                Ok(bytes) => {
                    tracing::debug!(
                        "stream {track_id} assembled {count} segments into {} bytes",
                        bytes.len()
                    );
                    audio_reply(bytes, range, attachment)
                }
                Err(e) => {
...
// Serve a segmented FLAC track as one native FLAC stream.
//
// No size sweep and no Content-Length: the rewrap strips MP4 box
// headers, so the final length is not knowable without fetching
// everything, and waiting for that is the delay this exists to remove.
// A FLAC stream needs neither — STREAMINFO carries the sample count, so
// the player knows the duration from the first few bytes and decodes
// forward from there.
//
// The whole stream is accumulated as it goes and cached on completion,
// so the next request for the same track serves from memory with an
// exact length and full range support. A client that disconnects early
...
async fn flac_reply(
    ...
    let Some(sizes) = flac_sizes(track_id, tier, &segments).await else {
        ...
        return match assemble_flac(track_id, tier, header, segments).await {
            Ok(bytes) => audio_reply_typed(bytes, range, attachment, "audio/flac"),
            Err(e) => {
                tracing::error!("track {track_id} could not be assembled: {e}");
                fail(0, "Stream unavailable").into_response()
            }
    ...
    // Part 0 is the header, held in memory; the rest are segments. The
    // stream is exactly those concatenated, so a byte offset maps onto
    // them the same way it does for any file.
    let total: u64 = header.len() as u64 + sizes.iter().sum::<u64>();
    ...
    let mut lens = Vec::with_capacity(sizes.len() + 1);
    lens.push(header.len() as u64);
    lens.extend(sizes.iter().copied());

    tracing::debug!(
        "stream {track_id} flac bytes {start}-{end}/{total} \
         ({} segments, {} byte header)",
        segments.len(),
        header.len()
    );

    // Which pieces the requested span touches, and how much of each.
    let mut wanted: Vec<(Option<String>, usize, usize)> = Vec::new();
    let mut offset = 0u64;
    for (idx, &len) in lens.iter().enumerate() {
        ...
        if piece_end > start && offset <= end {
            let from = start.saturating_sub(offset);
            let to = (end - offset + 1).min(len);
            let url = (idx > 0).then(|| segments[idx - 1].clone());
            wanted.push((url, from as usize, (to - from) as usize));
        }
        offset = piece_end;
        if offset > end {
            break;
    ...
    // continuous stream while later segments are still in flight.
    use futures_util::StreamExt as _;
    let body = futures_util::stream::iter(wanted)
        .map(move |(url, skip, take): (Option<String>, usize, usize)| {
            let header = header.clone();
            async move {
                match url {
                    // The header is already in hand.
                    None => Ok(bytes::Bytes::from(header[skip..skip + take].to_vec())),
                    Some(url) => {
                        let raw = fetch_part(&url).await?;
                        let frames = crate::flac::frames(&raw);
                        // A segment shorter than its header advertised
                        // would shift every later offset; stop rather
                        // than send audio that contradicts
                        // Content-Length.
                        if skip + take > frames.len() {
                            return Err(Error::Auth(format!(
                                "track {track_id}: segment shorter than its header claimed"
                            )));
                        }
                        Ok(bytes::Bytes::from(frames[skip..skip + take].to_vec()))
                    }
                }
            }
        })
        ...
        .chain(futures_util::stream::once(async move {
            if cacheable {
                ...
                tracing::debug!(
                    "stream {track_id} flac complete: {} bytes in {:?}",
                    bytes.len(),
                    began.elapsed()
                );
                store_audio(track_id, tier, &bytes);
...
async fn flac_sizes(track_id: u64, tier: Quality, segments: &[String]) -> Option<Arc<Vec<u64>>> {
    if let Some(sizes) = SIZE_CACHE.lock().unwrap().get(&(track_id, tier)) {
        return Some(Arc::clone(sizes));
    }
    ...
    SIZE_CACHE
        .lock()
        .unwrap()
        .insert((track_id, tier), Arc::clone(&sizes));
    Some(sizes)
}
...
// audio_reply with an explicit content type, for the rewrapped FLAC
// cache path where the body is no longer MP4.
fn audio_reply_typed(
    bytes: Arc<Vec<u8>>,
    range: Option<&str>,
    attachment: Option<String>,
    content_type: &str,
) -> warp::reply::Response {
    let mut resp = audio_reply(bytes, range, attachment);
    resp.headers_mut()
...
// Stream the requested byte range, fetching only the segments that
// cover it and sending each on as it arrives.
//
...
// one segment rather than after all ninety. Seeking still works because
// the size table gives an exact Content-Length, and a seek deep into a
// track now skips the segments before it instead of downloading them.
fn progressive_reply(
    track_id: u64,
    tier: Quality,
    urls: Vec<String>,
    sizes: Arc<Vec<u64>>,
    ...
    let parts = plan(&urls, &sizes, start, end);
    let length = end - start + 1;
    ...
    let warm = {
        let map = SEGMENT_CACHE.lock().unwrap();
        parts.iter().filter(|p| map.contains_key(&p.url)).count()
    };
    tracing::debug!(
        "stream {track_id} streaming bytes {start}-{end}/{total} from {} of {} segments ({warm} cached)",
        parts.len(),
        urls.len()
    );
    ...
    let body = futures_util::stream::unfold(
        (parts.into_iter(), Vec::new(), cacheable, 0usize),
        move |(mut parts, mut acc, cacheable, sent)| async move {
            let Some(part) = parts.next() else {
                // Exhausted: store the assembled track so repeats skip
                // the network entirely.
                if cacheable {
                    store_audio(track_id, tier, &Arc::new(std::mem::take(&mut acc)));
                }
                tracing::debug!(
                    "stream {track_id} body complete: {sent} bytes in {:?}",
                    began.elapsed()
                );
                return None;
            };
            let fetched = fetch_part(&part.url).await;
            let chunk = match fetched {
                Ok(bytes) => {
                    // A part shorter than its advertised length would
                    // shift every later offset; stop rather than send
                    // audio that does not line up with Content-Length.
                    if part.skip + part.take > bytes.len() {
                        Err(Error::Auth(format!(
                            "segment for track {track_id} was shorter than its HEAD reported"
                        )))
                    } else {
                        Ok(bytes.slice(part.skip..part.skip + part.take))
                    }
                }
                Err(e) => Err(e),
            };
            match chunk {
                Ok(bytes) => {
                    ...
                    let n = bytes.len();
                    Some((Ok(bytes), (parts, acc, cacheable, sent + n)))
                }
                Err(e) => {
                    tracing::error!("stream {track_id} failed mid-body: {e}");
