...
- [x] Blend Tidal mixes into getPlaylists — pages/my_collection_my_mixes → rows[].modules[].pagedList.items[] (mixType entries, video mixes filtered out by title prefix) mapped to Playlist with mx<id> prefix, full image URL cover (MEDIUM→SMALL), owner TIDAL, duration 0 / created epoch placeholders; getPlaylist(mx…) → /mixes/{id}/items (same {item} wrapper as playlist items, reuses playlist_song_from_item); update/delete on mx ids fail "Mixes are read-only"; show_mixes setting (default true) in settings.toml; mixes live in a 5-min mix_cache (they regenerate, unlike the 6h meta_cache)
- [x] Implement indexBasedQueue endpoints (getPlayQueueByIndex/savePlayQueueByIndex) — Feishin prefers these once indexBasedQueue is advertised; thin variants over the play_state store: response key playQueueByIndex with currentIndex (queue position) instead of current (song id); save resolves currentIndex → track id; currentIndex optional
- [ ] Batch 5: scrobble middleware (PlayReporter trait + Last.fm + ListenBrainz reporters + flip scrobblingEnabled)
